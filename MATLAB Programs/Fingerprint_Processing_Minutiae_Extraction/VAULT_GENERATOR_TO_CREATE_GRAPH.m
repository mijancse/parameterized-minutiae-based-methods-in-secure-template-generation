    ...
    for i=1:size(Minutiae(:,1),1)
       ...
       for k=1:ChaffPointsOnCircumference
           ...
           if(k==SelectedChaffPoint)    %%%%<<<------------- 6th point chosen (clockwise)
               ...
               if( (Minutiae(i,4) + angle_offset)>=6.2832)   %%%% <<-------------- ANGLE ADDED 2.5 {{{VARIABLE}}}
                   ChaffMinutiae(ChaffIndex,4) = (Minutiae(i,4) + angle_offset) - 6.2832;
               else
                   ChaffMinutiae(ChaffIndex,4) = (Minutiae(i,4) + angle_offset);
               end

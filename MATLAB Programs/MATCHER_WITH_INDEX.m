...
path = 'H:\Education\Thesis\FVC 2004\DB1_B';
...
%         FileName = 'fp2.bmp';
        ...
        % Indexing Code Creation And Store
        query_template = Minutiae;
        Discretization_Threshold = .35;
        NumOfRefs = size(Ref_imgs,1);
        IndexCodeOfQueryImg = '';
        for i=1:size(Ref_imgs,1)
            Sx = match(query_template, Ref_imgs{i,2});
            if Sx<Discretization_Threshold
                IndexCodeOfQueryImg(i) = '0';
            else
                IndexCodeOfQueryImg(i) = '1';
            end
        end
        display(strcat(FileName(1:3),':  ','Template=',num2str(k),', Prg=',num2str((double(k)/80)*100)));
        display(strcat('Indx Code=',IndexCodeOfQueryImg));
end
